...
if uploaded_file is not None:
    ...
    # Let user select the date column
    date_col = st.selectbox("Select the date column", options=data.columns, index=0)

    # Let user select stock columns (multiple)
    stock_cols = st.multiselect(
        "Select stock columns for analysis",
        options=[col for col in data.columns if col != date_col],
        default=[col for col in data.columns if col != date_col][:5]  # default: first 5 non-date columns
    )

    if not stock_cols:
        st.error("Please select at least one stock column.")
        st.stop()

    data = data.dropna(subset=[date_col] + stock_cols)
else:
...
@st.cache_data
def calculate_returns(data, stock_cols, date_col):
    returns = data[stock_cols].pct_change().dropna()
    returns[date_col] = data[date_col][1:].values
    return returns

returns = calculate_returns(data, stock_cols, date_col)

# Calculate statistics
@st.cache_data
def calculate_stats(returns):
    mean_returns = returns[stock_cols].mean()
    cov_matrix = returns[stock_cols].cov()
...
cumulative_returns = pd.DataFrame({
    'Date': returns[date_col],
    'Min Volatility': (returns[stock_cols].mul(min_vol_portfolio['weights'], axis=1).sum(axis=1) + 1).cumprod() - 1,
    'Max Sharpe': (returns[stock_cols].mul(max_sharpe_portfolio['weights'], axis=1).sum(axis=1) + 1).cumprod() - 1
})
...
if page == "Price Trends":
    ...
    selected_stock = st.selectbox("Select Stock", stock_cols, key="price_trends_select")
...
if page == "Returns":
    ...
    selected_stock = st.selectbox("Select Stock", stock_cols, key="returns_select")
